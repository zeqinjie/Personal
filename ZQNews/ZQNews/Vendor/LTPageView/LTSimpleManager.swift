 ...
 public class LTSimpleManager: UIView {
    ...
    /* 獲取header的高度 */
    @objc public var kHeaderHeight: CGFloat = 0.0
    /* 动态改变header的高度 */
    @objc public var glt_headerHeight: CGFloat = 0.0 {
     ...
     /* 移除headerview */
//    @objc public func removeHeaderView() {
...
//    }
    ...
    public typealias LTSimpleButtonClickHandle = (UIButton) -> Void
    @objc public var simpleButtonClickHandle: LTSimpleButtonClickHandle?
    @objc public func buttonClickHandle(_ handle: LTSimpleButtonClickHandle?) {
        simpleButtonClickHandle = handle
    }
    ...
    //當前選擇
    @objc public var currentSelectIndex: Int = 0
 ...
 extension LTSimpleManager {
    ...
    @objc public func getPageTitleViewY(toView:UIView) ->CGPoint{
        var point = CGPoint.zero
        let pageTitleView = self.pageView.pageTitleView
        point = pageTitleView.convert(pageTitleView.frame.origin, to: toView)
        return point
    }
 }
 
 // MARK: - LTPageViewDelegate
 ...
 extension LTSimpleManager {
    private func pageViewDidSelectConfig()  {
        ...
        pageView.buttonClickBlock = {[weak self](btn) in
            guard let `self` = self else { return }
            self.simpleButtonClickHandle?(btn)
        }
    }
